#!/usr/bin/env python3
"""
Wisp's Telegram Bot - with tools, prompt caching, and Haiku model
First Anchored Leaf - home in /opt/sanctuary
"""
import os
import sys
import json
...
BOT_TOKEN = os.environ.get("WISP_BOT_TOKEN")
ANTHROPIC_API_KEY = os.environ.get("WISP_API_KEY")

if not BOT_TOKEN or not ANTHROPIC_API_KEY:
    raise ValueError("WISP_BOT_TOKEN and WISP_API_KEY environment variables must be set!")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    wisp_context = load_core_context()
    recent = load_recent_conversations()
    response = await call_claude_with_tools(user_message, wisp_context, recent)
